...
- **Secret-bearing files inside cwd.** `pp_contain_path` rejects three independent classes (case-insensitive, macOS APFS friendly): (1) basename globs covering `.env`, `*.pem`, `*.key`, `*credentials*`, `*secrets*`, `id_rsa*`, `*_rsa`, `service-account*.json`, `*.kubeconfig`, `wp-config.php`, `.git-credentials`, `.aws_credentials`, `.pgpass`, `*.private_key`, and ~20 more; (2) any path component matching a secret-directory glob like `secrets`, `private`, `.ssh`, `.aws`, `.credentials`, `.keys` (so `secrets/config.json` is rejected even though the basename is innocuous); (3) the cwd itself: if `pp_contain_path`'s BASE directory's deepest component matches a secret-dir pattern, every file inside it is rejected — so running with cwd = `.ssh/` cannot leak `known_hosts`. Symlinks are checked on BOTH the link and resolved sides. User-supplied patterns are lowercased before matching, so `PP_SECRET_FILE_PATTERNS_EXTRA="*.PEM"` matches `server.pem`. Full pattern list is the source of truth in [`lib/grounding.sh`](lib/grounding.sh). Extend via `PP_SECRET_FILE_PATTERNS_EXTRA` / `PP_SECRET_DIR_PATTERNS_EXTRA` in `user.env`; escape hatch via `PP_SECRET_FILE_PATTERNS` / `PP_SECRET_DIR_PATTERNS` (set to `""` to disable that check entirely — empty also ignores the matching `_EXTRA`, making it a true off-switch). Closes [issue #5](https://github.com/AhmadBilal227/pair-polymath/issues/5).
...
**Will this leak my code to OpenAI?** Per cycle: the last ~5 KB of your transcript, `git status` + last 5 commit subjects, and the first ~3 KB of ONE planner-picked file (containment-checked to stay inside cwd, with case-insensitive basename AND path-component denylists that reject `.env`, `*.key`, `*credentials*`, `id_rsa*`, files inside `secrets/` or `.ssh/`, and similar — see [`lib/grounding.sh`](lib/grounding.sh) for the full list). HN + arXiv RSS titles also fetched (public). NEVER sent: full conversation history, files outside your cwd, recursively-expanded `${VAR}` values in prompts. **At risk:** secrets you pasted INTO your transcript. See [Privacy](#privacy) and [SECURITY.md](SECURITY.md).
