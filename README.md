...
- **Secret-bearing files inside cwd.** `pp_contain_path` rejects two independent classes (case-insensitive, macOS APFS friendly): (1) basename globs covering `.env`, `*.pem`, `*.key`, `*credentials*`, `*secrets*`, `id_rsa*`, `*_rsa`, `service-account*.json`, `*.kubeconfig`, `wp-config.php`, `.git-credentials`, `.aws_credentials`, `.pgpass`, `*.private_key`, and ~20 more; (2) any path component matching a secret-directory glob like `secrets`, `private`, `.ssh`, `.aws`, `.credentials`, `.keys` (so `secrets/config.json` is rejected even though the basename is innocuous). Symlinks are checked on BOTH the link and resolved sides. Full pattern list is the source of truth in [`lib/grounding.sh`](lib/grounding.sh). Extend via `PP_SECRET_FILE_PATTERNS_EXTRA` / `PP_SECRET_DIR_PATTERNS_EXTRA` in `user.env`; escape hatch via `PP_SECRET_FILE_PATTERNS` / `PP_SECRET_DIR_PATTERNS` (set to `""` to disable that check entirely). Closes [issue #5](https://github.com/AhmadBilal227/pair-polymath/issues/5).
