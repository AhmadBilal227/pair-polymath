...
- Secret-file denylist: `pp_contain_path` rejects two independent classes (case-insensitive, so macOS APFS `.ENV` doesn't bypass): (1) basename matches against ~30 patterns (`.env`, `*.pem`, `*.key`, `*credentials*`, `*secrets*`, `id_rsa*`, `*_rsa`, `service-account*.json`, `*.kubeconfig`, `wp-config.php`, `.git-credentials`, `.aws_credentials`, `.pgpass`, `*.private_key`, and similar); (2) any path component matching a secret-directory glob (`secrets`, `private`, `.ssh`, `.aws`, `.credentials`, `.keys`, `.secrets`) — so `secrets/config.json` is rejected even with an innocuous basename. Symlinks are checked on BOTH sides (link basename AND resolved real path) to prevent basename-trust bypass. Pathname-expansion of patterns is suppressed (`set -f` discipline) so a cwd containing `prod.env` cannot make `*.env` collapse to a single literal. Full source-of-truth pattern list lives in [`lib/grounding.sh`](lib/grounding.sh). Configurable via `PP_SECRET_FILE_PATTERNS_EXTRA` / `PP_SECRET_DIR_PATTERNS_EXTRA` (additive) or `PP_SECRET_FILE_PATTERNS` / `PP_SECRET_DIR_PATTERNS` (replace; explicit `""` disables that check; whitespace-only logs a warning and reverts to defaults).
